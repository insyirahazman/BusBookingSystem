...
try {
    ...
    String formattedDate = dateOutputFormat.format(parsedDate);
    pst.setDate(4, java.sql.Date.valueOf(formattedDate));

    // Parse the time from "hh:mm:ss a" (12-hour format with AM/PM) to "HH:mm:ss" (24-hour format)
    ...
    java.util.Date parsedTime = timeInputFormat.parse(data[4].trim());
    String formattedTime = timeOutputFormat.format(parsedTime);
    pst.setTime(5, java.sql.Time.valueOf(formattedTime));
